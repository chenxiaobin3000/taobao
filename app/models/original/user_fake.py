...
# 刷单表
class UserFakeManager(models.Manager):
    def add(self, user_id, shop_id, order_id, payment, procure, order_status, create_time, good_ids, procure_ids, order_note):
        return self.create(user_id=user_id, shop_id=shop_id, order_id=order_id, payment=payment, procure=procure, order_status=order_status, create_time=create_time, good_ids=good_ids, procure_ids=procure_ids, order_note=order_note)
    ...
    def getById(self, user_id, shop_id, order_id):
        return self.encoder(self.encoder(self.filter(user_id=user_id, shop_id=shop_id, order_id=order_id).first()))
    ...
    def getList(self, user_id, shop_id, page, num):
        left = (page - 1) * num
        right = page * num
        return self.encoderList(self.encoderList(self.filter(user_id=user_id, shop_id=shop_id).order_by('-create_time')[left:right]))
