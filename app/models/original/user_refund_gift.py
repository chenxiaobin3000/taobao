...
# 退货赠品表
class UserRefundGiftManager(models.Manager):
    ...
    def getList(self, user_id, shop_id, page, num, start_date=None, end_date=None, search=None):
        left = (page - 1) * num
        right = page * num
        return self.encoderList(self.filterByDate(user_id, shop_id, start_date, end_date, search).order_by('-apply_time')[left:right])
...
class UserRefundGift(models.Model):
    objects = UserRefundGiftManager()
