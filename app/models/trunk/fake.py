...
class FakeManager(models.Manager):
    ...
    def getById(self, shop_id, order_id):
        return self.encoder(self.encoder(self.filter(shop_id=shop_id, order_id=order_id).first()))
    ...
    def getList(self, shop_id, page, num):
        left = (page - 1) * num
        right = page * num
        return self.encoderList(self.encoderList(self.filter(shop_id=shop_id).order_by('-create_time')[left:right]))

    def getListByDay(self, shop_id, start_date, end_date):
        return self.encoderList(self.filter(shop_id=shop_id, create_time__range=(start_date, end_date)).aggregate(models.Sum('payment'), models.Count('id')))
