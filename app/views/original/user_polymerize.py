...
def getList(request):
    ...
    total = UserPolymerize.objects.total(user_id, shop_id, start_date, end_date, search)
    polymerizes = UserPolymerize.objects.getList(user_id, shop_id, page, num, start_date, end_date, search)
    response = success({
            'total': total,
            'list': polymerizes
        })
